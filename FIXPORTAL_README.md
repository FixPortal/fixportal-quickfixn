...
| 1.14.1-fixportal.8 | 2026-09-07 | Prevents integer overflow in test-request thresholds, preserves settings-load exception causes, and rejects malformed or out-of-range ReconnectInterval values instead of silently using the default. Also fixes failed-connection socket disposal and stale accept callbacks in the dynamic-session test harness. |
| 1.14.1-fixportal.7 | 2026-09-01 | Adds the optional, host-neutral `IOutboundSendJournal` boundary for durable outbound frames. `Session.Send(string)` synchronously calls `Prepare(SessionID, rawFrame)` before the responder (a prepare failure prevents the send), then records the responder result as diagnostic-only outcome data; outcome failures are logged and suppressed, while responder throws leave the prepared record unresolved for recovery. The journal is threaded through `SessionFactory`, `SocketInitiator`, and `ThreadedSocketAcceptor`; `IFixWireTap` remains an observational post-responder diagnostic seam. Outbound message-store persistence now fails closed when its atomic `SetAndIncrNextSenderMsgSeqNum` reports failure, preventing both journal preparation and responder send. |
...
| 1.14.1-fixportal.5 | 2026-08-10 | `1e8014c6` (`feat: identify rejects of resent messages`): `IApplicationMessageRejection` gains a five-argument `OnMessageRejected` overload adding `bool referencedMessageWasResent`, default-implemented in terms of the existing four-argument method so prior implementers stay source-compatible. `Session` tracks successfully retransmitted outbound sequence numbers in a bounded 1,024-entry FIFO (`RecordResentOutbound`/`WasResentOutbound`/`ClearResentOutboundHistory`, capacity `ResentOutboundHistoryCapacity`), cleared alongside the existing `_resentTracker` on disconnect, `ResetOnLogon`, and a counterparty-requested reset, so a session-level Reject can tell the application whether the rejected message was originally sent as part of a resend. |
