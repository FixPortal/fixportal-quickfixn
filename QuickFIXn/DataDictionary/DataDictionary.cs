...
public class DataDictionary
{
    ...
    public void CheckValue(IField field)
    {
        ...
        if (FieldsByTag.TryGetValue(field.Tag, out var fld))
        {
            ...
            if (fld.HasEnums())
            {
                ...
                {
                    var invalidValues =
                        field.ToString().Split(' ').Where(value => !fld.EnumDict.ContainsKey(value)).ToList();

                    if (invalidValues.Count > 0)
                    {
                        throw new IncorrectTagValue(field.Tag, string.Join(",", invalidValues));
                    }
                }
    ...
    internal static string VerifyChildNodeAndReturnNameAtt(XmlNode childNode, XmlNode parentNode)
    {
        ...
        if (nameatt is null)
        {
            ...
            throw new DictionaryParseException(
                $"Malformed data dictionary: Found '{childNode.Name}' node without 'name' within parent '{parentNode.Name}/{messageTypeName}'");
        }
        ...
        foreach (XmlNode childNode in node.ChildNodes)
        {
            ...
            */
            ...
            switch (childNode.Name)
            {
                ...
                default:
                    throw new DictionaryParseException(
                        $"Malformed data dictionary: child node type should be one of {{field,group,component}} but is '{childNode.Name}' within parent '{node.Name}/{messageTypeName}'");
            }
        }
    }
}
